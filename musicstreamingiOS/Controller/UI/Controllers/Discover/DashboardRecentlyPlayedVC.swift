//
//  DashboardRecentlyPlayedVC.swift
//  DeepSoundiOS
//
...
class DashboardRecentlyPlayedVC: BaseVC {
    ...
    var recentlyPlayedArray:DiscoverModel.NewReleasesRecentlyPlayed?
    private let popupContentController = R.storyboard.player.musicPlayerVC()
    private var recentlPlayedMusicArray = [MusicPlayerModel]()
    ...
    private func setupUI(){
        self.title = NSLocalizedString("Recently Played", comment: "Recently Played")
        
        self.showImage.isHidden = true
        self.showLabel.isHidden = true
        self.tableView.separatorStyle = .none
        tableView.register(R.nib.dashboardRecentlyPlayedTableCell(), forCellReuseIdentifier: R.reuseIdentifier.dashboardRecentlyPlayed_TableCell.identifier)
        if (self.recentlyPlayedArray?.data!.isEmpty) ?? false{
        ...
        for (value) in  self.recentlyPlayedArray!.data!{
            self.songObject.append(value)
          }
    } 
}
extension DashboardRecentlyPlayedVC:UITableViewDelegate,UITableViewDataSource{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return (self.recentlyPlayedArray?.data!.count) ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: R.reuseIdentifier.dashboardRecentlyPlayed_TableCell.identifier) as? DashboardRecentlyPlayed_TableCell
        cell?.selectionStyle = .none
        let object = self.songObject[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let object = songObject[indexPath.row]
        AppInstance.instance.player = nil
        AppInstance.instance.AlreadyPlayed = false
        
        self.songObject.forEach({ (it) in
            ...
            self.recentlPlayedMusicArray.append(musicObject)
        })
        var audioString:String? = ""
        var isDemo:Bool? = false
        ...
        popupContentController!.popupItem.subtitle =
            object.title?.htmlAttributedString ?? ""
        
        let cell  = tableView.cellForRow(at: indexPath) as? DashboardRecentlyPlayed_TableCell
        ...
        tabBarController?.presentPopupBar(withContentViewController: popupContentController!, animated: true, completion: {
            ...
            self.popupContentController!.musicArray = self.recentlPlayedMusicArray
...
extension DashboardRecentlyPlayedVC:likeDislikeSongDelegate{
